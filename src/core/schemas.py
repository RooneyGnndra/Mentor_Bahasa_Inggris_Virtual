...
class ListeningExerciseSchema(BaseModel):
    ...
    questions: list[str] = Field(
        ..., description="Daftar pertanyaan untuk menguji peserta berdasarkan 'script'"
    )


class EvaluateUserIntentionSchema(BaseModel):
    skill_types: Literal["reading", "speaking", "writing", "listening"] = Field(
        ..., description="pilihan salah satu skiil types yang dibutuhkan peserta"
    )
...
class LearningSkillTypesSchema(BaseModel):  # item latihan yang dilakukan peserta
    category: str = Field(
        ...,
        description="salah satu kategori skill types: reading, speaking, writing dan listening",
    )
    title: str = Field(..., description="judul latihan")
    feedback: str = Field(
        ..., description="penilaian objektif dengan metode sandwich feedback"
    )
...
class LearningReportSchema(BaseModel):  # laporan belajar
    ...
    skill_types: list[LearningSkillTypesSchema]
